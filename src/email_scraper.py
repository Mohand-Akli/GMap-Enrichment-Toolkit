        ...
        try:
            with DDGS() as ddgs:
                ...
                for res in resultats:
                    # ASTUCE EN OR : Chercher l'email DIRECTEMENT dans la description de la recherche
                    # (Parfait pour les pages Facebook/Instagram qu'on ne peut pas scraper directement)
...
def scraper_emails_rapide(fichier_entree, fichier_sortie):
    ...
    if 'Restaurant_name' not in df.columns:
        print(f"❌ Erreur : Colonne 'Restaurant_name' introuvable.")
        return

    # S'assurer que les colonnes adress et website existent (même vides) pour éviter les crashs
    if 'adress' not in df.columns: df['adress'] = ""
